        ...
        public static List<string> WriteToFile<T>(string filePath, char delimeter, TypeConverter typeConverter, IEnumerable<T> mods, Type type, Type parentClass, bool isHaveHeaders = true, bool isHaveQuotes = true)
        {
            ...
            if (models.Count > 0)
            {
                using (StreamWriter sw = new StreamWriter(filePath))
                {
                    if (isHaveHeaders)
                    {
                        sw.WriteLine(GenerateHeaders(DictionaryCreator.CreateFromModel(models[0], type, parentClass, typeConverter), delimeter));
                    }

                    foreach (Object model in models)
                    {
                        sw.WriteLine(GenerateLine(DictionaryCreator.CreateFromModel(model, type, parentClass, typeConverter), delimeter, isHaveQuotes));
                    }
                }
            }
